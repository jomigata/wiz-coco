...
def _load_assessments_for_portal_ecosystem(db, primary_portal_id: str):
    """현재 + 연결된 나의코드의 상담(코드) 목록(중복 assessment 제거)."""
    items = []
    seen = set()
...
def list_counselor_assignments():
    """상담사 내담자 검사 할당 목록 (포털×상담(코드)×검사항목)."""
    status = (request.args.get("status") or "active").strip().lower()
    if status not in ("active", "archived", "all"):
        status = "active"
...
def portal_login():
    ...
    # 공유 상담(코드)는 검사 시작 전용 — 내 검사실은 개인 포털(코드+PIN)로만 접속
...
def bulk_create():
    ...
    public_claim_channel = (body.get("publicClaimChannel") or body.get("public_claim_channel") or "").strip()
    ...
    try:
        assessment_ref_id, join_access_code, cohort_id = prepare_bulk_assessment(
            ...
            public_claim_channel=public_claim_channel,
        )
    except PermissionError as exc:
        return jsonify({"error": "Forbidden", "message": str(exc)}), 403
    except ValueError as exc:
...
def resend_dispatch(assessment_id):
    """선택 내담자에게 상담(코드)·나의코드·비밀번호 재발송 (비밀번호 재발급)."""
    body = request.get_json(silent=True) or {}
    portal_ids = body.get("portalIds") or []
    if not isinstance(portal_ids, list) or not portal_ids:
        return jsonify({"error": "Bad Request", "message": "portalIds가 필요합니다."}), 400
...
def bulk_export_csv():
    ...
    writer.writerow(["이름", "이메일", "휴대폰", "상담(코드)", "나의코드", "비밀번호", "매직링크경로"])
    for r in rows:
        writer.writerow(
            [
                r.get("displayName", ""),
