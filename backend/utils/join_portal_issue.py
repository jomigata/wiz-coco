"""공동 상담(코드) 참여자 — 검사 1건 이상 완료 시 개인 포털(코드+PIN) 발급."""
...
def try_issue_portal_for_participant(db, participant_id: str, assessment_id: str) -> dict:
    """
    공동 상담(코드) 참여자에게 검사 1건 이상 완료 시 내 검사실 자격 증명 발급.
    ...
    if not ass_doc.exists:
        return {"ok": False, "error": "assessment_not_found", "message": "상담(코드)를 찾을 수 없습니다."}
    ...
    if issue_type != "shared":
        return {
            ...
            "skipped": True,
            "message": "개별 발급 상담(코드)는 발급 시 접속 정보가 전달됩니다.",
        }
