...
| `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `MAIL_FROM` | (선택) 상담사 승인 요청 등 관리자 알림 메일. 미설정 시 발송 안 함 |
| `COUNSELOR_ADMIN_NOTIFY_EMAIL` | 상담사 전환 승인 요청 수신 (기본 `jomigata@gmail.com`) |
...
| **Secret Manager Admin** (`roles/secretmanager.admin`) 또는 Secret 생성·버전 추가·IAM 권한 | SMTP 비밀번호를 Secret Manager에 저장 (deploy-backend.yml) |
...
GitHub **Settings → Secrets and variables → Actions** 에 아래 Secret을 등록하면 `deploy-backend.yml`이 Cloud Run 환경 변수로 주입합니다.
...
| `SMTP_PASSWORD` | Google **앱 비밀번호** (2단계 인증 후 생성) |
| `MAIL_FROM` | 발송 표시 주소 (보통 `SMTP_USER`와 동일) |
| `COUNSELOR_ADMIN_NOTIFY_EMAIL` | `jomigata@gmail.com` (미설정 시 기본값) |

로컬 개발: `backend/.env`에 동일 변수 설정 후 Flask 재시작.
