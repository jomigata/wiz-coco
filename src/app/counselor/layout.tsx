...
export default function CounselorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div className={`flex min-h-[100dvh] flex-col text-slate-900 ${isHubPage ? counselorHubClasses.page : 'bg-[#f8fafc]'}`}>
      
<div className="flex min-h-0 flex-1 flex-col pt-16">
        {/* 페이지 상단 */}
        <header className={`shrink-0 border-b py-2.5 backdrop-blur-sm ${
          isHubPage ? 'border-slate-200 bg-white/90' : 'border-slate-200 bg-white/90'
        }`}>
          <div className="mx-auto flex w-full max-w-[1800px] items-center justify-between gap-3 px-4 sm:px-6">
            ...
            <div className="flex shrink-0 items-center gap-2 sm:gap-3">
              <div className="hidden text-right sm:block">
                <p className="text-[10px] uppercase tracking-wide text-slate-400">상담사</p>
                <p className="text-sm font-medium text-slate-700">김상담</p>
              </div>
              <div className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-slate-50 sm:h-9 sm:w-9">
                <span className="text-xs font-semibold text-slate-600 sm:text-sm">김</span>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className={`relative flex min-h-0 flex-1 flex-col overflow-hidden ${
          isHubPage ? counselorHubClasses.page : 'bg-[#f8fafc]'
        }`}>
          <div className={`pointer-events-none absolute inset-0 ${
            isHubPage
              ? counselorHubClasses.pageGlow
              : 'bg-[radial-gradient(ellipse_80%_50%_at_50%_-10%,rgba(37,99,235,0.05),transparent)]'
          }`} />
