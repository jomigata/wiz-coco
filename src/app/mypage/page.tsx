...
function TestRecordsTabContent({
  ...
  const openCounselorView = async (record: TestRecord) => {
    ...
    try {
      ...
      const setTitle = record.testType?.replace(/^상담사 상담(코드) · /, '') || '—';
      const lines = [
        `검사 세트: ${setTitle}`,
        `상담(코드): ${formatCodePinDisplay(record)}`,
  ...
  const getDisplayTestName = (record: TestRecord): string => {
    if (record.recordSource === 'counselor-assessment') {
      ...
      const setTitle = record.testType?.replace(/^상담사 상담(코드) · /, '').trim();
  ...
  const getAccessCodeTitleLabel = (record: TestRecord): string => {
    // 괄호 안: 상담사 상담(코드) 플로우는 "안내 제목(세트 제목)"을 표시
    if (record.recordSource === 'counselor-assessment') {
      const setTitle = record.testType?.replace(/^상담사 상담(코드) · /, '').trim();
      return setTitle || '상담사 상담(코드)';
    }
    // 개인 기록은 수행할 검사명 사용
  ...
  const getAccessCodeGroupTitle = (accessCode: string, children: TestRecord[]): string => {
    ...
    const titles = children
      .map((c) => (c.testType || '').replace(/^상담사 상담(코드) · /, '').trim())
      .filter(Boolean);
    const t = titles.find((x) => x && x !== '상담사 상담(코드)') || titles[0] || '상담사 상담(코드)';
    return t;
  };
  ...
  const handleRecordClick = (record: TestRecord) => {
    ...
    if (!record.code) {
      console.warn('상담(코드)가 없어 결과 페이지로 이동할 수 없습니다:', record);
      return;
    }
  ...
  const toggleAllSelection = () => {
    // 그룹 행은 체크 대상에서 제외(상담사 상담(코드) 일괄삭제 불가)
            ...
            <table className="min-w-full divide-y divide-white/10 text-sm">
              <thead className="sticky top-0 z-[1] bg-[#0f172a]/95 backdrop-blur-sm">
                <tr>
                  ...
                  <th
                    scope="col"
                    className="cursor-pointer px-2 py-2 text-left text-xs font-medium text-slate-400 hover:text-slate-200"
                                ...
                                <div className="flex items-center justify-center gap-1.5">
                                  {record.status === 'completed' ? (
                                    ...
                                    <button
                                      ...
                                      title="클릭하여 검사하기로 이동 (상담(코드) 자동 입력)"
                                      onClick={() => handleAddTestForRecord(record)}
                                    >
                                      ➕ 미검사
                                    </button>
              ...
              <div className="bg-white/5 rounded-lg p-4 space-y-2">
                ...
                {(deleteModalRecord.counselorCodePinDisplay || deleteModalRecord.counselorCode) && (
                  <div className="flex justify-between gap-2">
                    <span className="text-blue-300 shrink-0">상담(코드):</span>
