  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!code || !isValidAccessCodeInput(code)) {
        setAccessCheckError('잘못된 상담(코드)입니다.');
      ...
      try {
        ...
        setAccessCheckError(err instanceof Error ? err.message : '상담(코드)를 사용할 수 없습니다.');
      } finally {
        setAccessCheckLoading(false);
      }
    };
        ...
        <main className="max-w-2xl mx-auto">
          ...
          <div className="bg-slate-800/80 rounded-2xl border border-slate-600 p-6 shadow-xl">
            ...
            {!hasPortal && !hasParticipant && accessCheckLoading ? (
              ...
              <p className="text-amber-400/95 text-sm mb-4">검사 세션을 시작할 수 없습니다. 상담(코드) 입력부터 다시 시도해 주세요.</p>
            ) : null}
