 ...
 * 상담사 중심 상담(코드) 플랫폼: assessments, testResults 컬렉션
 */
...
export interface Assessment {
  /** 유일 상담(코드) (신규: CVC+숫자 3자리~, 구형: 영숫자 6자리) */
  accessCode: string;
  /** @deprecated 구 데이터 — 신규 상담(코드)는 PIN 없음 */
  joinPinHash?: string;
  /** @deprecated */
  joinPin?: string;
  /** 상담사 Firebase UID */
...
export interface TestResult {
  /** 상담(코드) (assessments.accessCode와 동일 규칙) */
  accessCode: string;
  /** assessment 문서 ID */
  assessmentId: string;
  /** 검사 ID */
