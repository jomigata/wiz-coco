...
export default function BusinessLegalBlock({
  ...
  if (variant === 'full') {
    return (
      <div
        className={`rounded-xl border border-slate-200 bg-white p-5 text-xs leading-relaxed text-slate-600 shadow-sm ${className}`}
      >
        ...
        <dl className="grid gap-2 sm:grid-cols-[7.5rem_1fr]">
          ...
          <dd className="text-slate-700">{b.companyName}</dd>
          <dt className="text-slate-500">대표자</dt>
          <dd className="text-slate-700">{b.representative}</dd>
          <dt className="text-slate-500">사업자등록번호</dt>
          <dd className="text-slate-700">{b.businessRegistrationNumber}</dd>
          <dt className="text-slate-500">사업장 소재지</dt>
          <dd className="text-slate-700">{b.address}</dd>
  ...
  return (
    <div className={`space-y-1 text-[11px] leading-relaxed text-slate-500 ${className}`}>
      <p>
        <span className="text-slate-600">{b.brandNameKo}</span> · 상호 {b.companyName} · 대표{' '}
        {b.representative}
      </p>
      <p>
