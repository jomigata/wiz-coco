    ...
    >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-900 border border-white/10 rounded-xl max-w-md w-full p-6 shadow-2xl text-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-semibold text-white">상담(코드) 삭제</h2>
            </div>
            ...
            <p className="text-slate-400 mb-5 leading-relaxed">
              목록에서 제거되어 삭제된 상담(코드) 목록으로 이동합니다. 복구하면 상담(코드) 목록에 다시 표시됩니다. 이미 제출된 결과는 상담사 화면에서 계속 조회할 수 있습니다.
            </p>
      ...
      {createdInfo && (
        <div className="mb-2 shrink-0 rounded-lg border border-emerald-500/30 bg-emerald-950/40 px-3 py-2">
          <p className="text-emerald-200 font-medium">상담(코드)가 발급되었습니다</p>
      ...
      {filtered.length === 0 ? (
        <div className="flex min-h-[12rem] flex-1 flex-col items-center justify-center rounded-md border border-white/10 bg-white/[0.03] py-10 text-center">
          ...
          <p className="text-base text-slate-300">
            {listItems.length === 0 ? '등록된 상담(코드)가 없습니다' : '검색 결과가 없습니다'}
          </p>
          <p className="mt-1 text-sm text-slate-500">
            {listItems.length === 0 ? '첫 상담(코드)를 만들어 내담자에게 배포하세요.' : '검색어를 바꿔 보세요.'}
          </p>
          {listItems.length === 0 && (
            <AuthLink
              href="/counselor/assessments/new"
