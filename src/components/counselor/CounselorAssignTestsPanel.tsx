...
export default function CounselorAssignTestsPanel() {
  ...
  return (
    <CounselorPageSection
      ...
      title="검사 할당 현황"
      description="발급된 상담패키지에 연결된 내담자별 검사 항목 진행 현황입니다. 새 내담자·검사 발급은 상담패키지 구성에서 진행하세요."
      toolbar={
        <>
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <h3 className="text-sm font-medium text-slate-200">신규 내담자 + 상담패키지</h3>
          <p className="mt-1 text-xs text-slate-500">
            상담패키지를 구성하면서 내담자 나의코드·PIN을 함께 발급합니다.
          </p>
          <AuthLink
            href="/counselor/assessments/new"
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <h3 className="text-sm font-medium text-slate-200">기존 상담패키지에 내담자 추가</h3>
          <p className="mt-1 text-xs text-slate-500">
            이미 만든 상담패키지에 수신자를 추가할 때는 만들기 화면에서 기존 코드를 선택하세요.
          </p>
          <AuthLink
            href="/counselor/assessments/new"
            className="mt-3 inline-flex text-sm text-sky-400 hover:text-sky-300"
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] px-6 py-14 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            상담패키지를 발급하면 내담자별 검사 항목이 여기에 표시됩니다.
          </p>
          <AuthLink
            href="/counselor/assessments/new"
          ...
          <table className="min-w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/10 text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-4 py-3 font-medium">상담패키지</th>
