  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
        ...
        <span>
          상담(코드) <strong className="text-slate-200">{item.assessmentCount}</strong>건
        </span>
        <span>
      ...
      {item.assessments.length > 0 ? (
        <div className="mt-4 space-y-2">
          <p className="text-xs font-medium text-slate-500">상담(코드)별 진행</p>
    ...
    <div className="space-y-6">
      ...
      {data.summary.notifyFailedCount > 0 ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          자격증명 발송 실패 {data.summary.notifyFailedCount}건 — 상담(코드)별 「발송·검사 현황」에서 재발송할 수
          있습니다.
        </div>
      ) : null}
        ...
        {data.cohorts.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.03] px-6 py-14 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              상담(코드)를 발급하고 내담자를 배정하면 그룹별 진행이 표시됩니다.
            </p>
          </div>
        ) : (
