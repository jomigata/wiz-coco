  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
        ...
        <span>
          상담코드 <strong className="text-slate-200">{item.assessmentCount}</strong>건
        </span>
        <span>
      ...
      {item.assessments.length > 0 ? (
        <div className="mt-4 space-y-2">
          <p className="text-xs font-medium text-slate-500">상담코드별 진행</p>
        ...
        {data.cohorts.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/[0.03] px-6 py-14 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              상담코드를 발급하고 내담자를 배정하면 그룹별 진행이 표시됩니다.
            </p>
          </div>
        ) : (
