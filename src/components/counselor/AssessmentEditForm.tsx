    ...
    <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col gap-4">
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 xl:grid-cols-2 xl:items-start">
        <div ref={leftColRef} className="flex min-h-0 flex-col gap-4">
          <CounselorPageSection title="상담패키지">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="text-sm text-slate-400">
                {initial.issueType === 'individual' ? '상담패키지(개별 발급)' : '일반코드(지원 종료)'}
              </span>
            </div>
            <p className={`${FORM_HINT} mt-2`}>상담패키지·발급 유형은 변경할 수 없습니다.</p>
