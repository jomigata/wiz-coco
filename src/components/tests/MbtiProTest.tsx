...
import { buildMbtiProJoinResponses, mbtiProResponsesChanged, parseMbtiProJoinResponses } from '@/lib/mbtiProJoinResponses';
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (hasPortal) {
          ...
          try {
            ...
            if (joinFromPortal) {
              ...
              throw new Error('이 상담(코드)는 현재 로그인한 나의코드에 연결되어 있지 않습니다.');
            }
          }
        } else {
          await ensureJoinGuestSession(code);
        ...
        if (!cancelled) {
          setJoinAccessError(err instanceof Error ? err.message : '상담(코드)를 사용할 수 없습니다.');
        }
      } finally {
  ...
  const variants = {
    enter: (dir: number) => ({
      ...
      transition: {
        ...
        opacity: {
          duration: QUESTION_TRANSITION_DURATION * 0.9,
          ease: QUESTION_ENTER_EASE,
          delay: QUESTION_ENTER_DELAY,
        },
        scale: {
            ...
            <div className="text-center mb-3">
              ...
              <div className="flex justify-between mt-3">
                ...
                <button
                  ...
                  className={`px-5 py-2 text-sm rounded-lg font-medium transition-all duration-300 ${
