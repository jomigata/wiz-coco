/**
 * 상담(코드) 참여 세션: 브라우저 sessionStorage에 세트 메타만 보관합니다.
 */
...
/** @deprecated 상담(코드) 직접 입력 제거 — 검사시작(포털 로그인) */
...
export async function startJoinAssessmentFromAccessCode(
  ...
  const code = normalizeAccessCodeInput(accessCodeNorm);
  if (!code) {
    return { ok: false, error: '상담(코드)가 없습니다.' };
  }
  try {
    ...
    return {
      ok: false,
      error: err instanceof Error ? err.message : '상담(코드) 확인에 실패했습니다.',
    };
  }
}
