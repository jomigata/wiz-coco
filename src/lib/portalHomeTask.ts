...
export function pickPortalHomeTask(params: {
  ...
  for (const assessment of assessments) {
    ...
    for (const test of assessment.testList || []) {
      totalTests += 1;
      const completed = isTestCompleted(String(test.testId), results);
      if (completed) {
        completedTests += 1;
        continue;
      }
      const inProgress = findInProgressResult(String(test.testId), results);
      return {
        kind: 'test',
        assessmentId: assessment.assessmentId,
        accessCode: assessment.accessCode,
        assessmentTitle: (assessment.title || '').trim() || '검사',
        testId: String(test.testId),
        testName: test.name || String(test.testId),
        mode: inProgress ? 'continue' : 'start',
  ...
  if (totalTests > 0 && completedTests >= totalTests) {
    return { kind: 'all_done', completedTests, totalTests };
  }
