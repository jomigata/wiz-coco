...
| # | 작업 | 상태 |
|---|------|------|
| 3.1 | `org_admin` 역할·기관 대시보드 | 완료 |
| 3.2 | 기관 선결제·0원 입장 (org 크레딧·bulk) | 완료 |
| 3.3 | 익명 그룹 통계 리포트 + PDF | 완료 |
| 3.4 | Admin 기관 POC 관리 (`/admin/organizations/`) | 완료 |

### POC 운영 순서

1. Admin → **B2B 기관 관리**에서 기관 생성 + liaison 상담사 UID
2. 기관에 크레딧 30~100 지급 (학급 단위)
3. org_admin UID 배정 (또는 생성 시 adminUid)
4. org_admin → **일괄 발송** → 내담자 0원
5. **익명 그룹 리포트** PDF 출력

경로: `/org/dashboard/`, `/org/dispatch/`, `/org/reports/?cohortId=…`
