...
exports.searchCounselFaqs = searchCounselFaqs;
exports.formatFaqKnowledgeContext = formatFaqKnowledgeContext;
...
function scoreFaq(faq, message, tokens) {
    ...
    for (const kw of faq.keywords) {
        ...
        if (lowerMsg.includes(k))
            score += 2;
        if (tokens.some((t) => t.includes(k) || k.includes(t)))
            score += 1;
    }
...
async function searchCounselFaqs(userMessage) {
...
function formatFaqKnowledgeContext(faqs) {
    if (faqs.length === 0)
        return undefined;
    const parts = [
